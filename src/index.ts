...
async function main() {
	...
	model.add(tf.layers.dense({units: 1024, activation: 'sigmoid', inputShape: [features[0].length]}));
	model.add(tf.layers.dense({units: 528, activation: 'sigmoid'}));
	...
	const saveLocation = path.resolve('../normal-model')
